...
function Pool (path, args, options, settings) {
    ...
    this.busy = []; // keep a record of busy children
    ...
    instance.acquire(function (err, client) {
        if (err) {
            ...
            client.once('message', function (message) {
                ...
                _.remove(self.busy, function(child) {
                    child.pid === client.pid;
                });
...
Pool.prototype.drain = function (callback, nuke) {
    var instance = this.pool;
    if (nuke) {
        var child;
        while ((child = this.busy.shift())) {
                instance.release(child);
        }
    }

    instance.drain(function() {
        instance.destroyAllNow(callback);
    });
};
